...
open class Token {
    ...
    final class StartTag: Tag {
        @usableFromInline
        var resolvedTag: SwiftSoupTag? = nil
        @usableFromInline
        ...
        @inline(__always)
        func resolveTag(_ settings: ParseSettings, isSelfClosing: Bool) throws -> SwiftSoupTag {
            if let cached = resolvedTag,
               resolvedTagPreservesCase == settings.preservesTagCase(),
               resolvedTagIsSelfClosing == isSelfClosing {
                return cached
            }
            let resolved: SwiftSoupTag
            if settings.preservesTagCase() {
                resolved = try SwiftSoupTag.valueOf(try name(), settings, isSelfClosing: isSelfClosing)
            } else if let fastTag = SwiftSoupTag.valueOfTagId(tagId) {
                resolved = fastTag
            } else if let normalName = normalName() {
                resolved = try SwiftSoupTag.valueOfNormalized(normalName, isSelfClosing: isSelfClosing)
            } else {
                resolved = try SwiftSoupTag.valueOf(try name(), settings, isSelfClosing: isSelfClosing)
            }
